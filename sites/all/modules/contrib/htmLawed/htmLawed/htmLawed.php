...
/*
htmLawed 1.1.4, 28 January 2009
...
foreach($aA as $k=>$v){
 if(!isset($C['deny_attribute'][$k]) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e])))){
  ...
  if($k == 'style'){
   ...
   $v = !$C['css_expression'] ? (preg_match('`expression`i', preg_replace('`/\*.*?\*/`S', '', html_entity_decode(urldecode(($v = preg_replace('`expression`i', '', $v)))))) ? '' : $v) : $v;
